...
def main():
    ...
    for file_path in file_path_li:
        ...
        if ("no-migration" not in workload_signature)\
            ...
            for num_accessed_pages, num_total_pages in zip(num_accessed_pages_li, num_total_pages_li):
                if num_total_pages != 0:
                    accessed_pages_ratio = (num_accessed_pages * 100) / num_total_pages
                else:
                    accessed_pages_ratio = 0
                data_dict[abbr_workload_name][thp][mig_policy].append(accessed_pages_ratio)
    ...
    for abbr_workload_name in data_dict.keys():
        for thp in data_dict[abbr_workload_name].keys():
            for mig_policy in data_dict[abbr_workload_name][thp].keys():
                accessed_pages_ratio_li = data_dict[abbr_workload_name][thp][mig_policy]
                result = "%s,%s,%s,%s"\
                        % (abbr_workload_name, thp, mig_policy,
                                ",".join(list(map(str, accessed_pages_ratio_li))))
                print(result)
