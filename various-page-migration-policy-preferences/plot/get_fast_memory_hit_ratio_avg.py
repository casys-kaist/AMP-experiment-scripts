...
"""
   get_fast_memory_hit_ratio_avg.py
...
def main():
    ...
    for file_path in file_path_li:
        ...
        if ("no-migration" not in workload_signature)\
            ...
            num_fast_memory_hit_pages = sum(num_fast_memory_hit_pages_li)
            ...
            data_dict[abbr_workload_name][thp].update({mig_policy: fast_memory_hit_ratio})
    ...
    for abbr_workload_name in data_dict.keys():
        for thp in data_dict[abbr_workload_name].keys():
            for mig_policy in data_dict[abbr_workload_name][thp].keys():
                fast_memory_hit_ratio = data_dict[abbr_workload_name][thp][mig_policy]
                result = "%s,%s,%s,%f" % (abbr_workload_name, thp, mig_policy, fast_memory_hit_ratio)
                print(result)
